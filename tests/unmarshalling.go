...
func TestRecursiveUnmarshal(t *testing.T, nb ipld.NodeBuilder) {
	t.Run("short list node", func(t *testing.T) {
		...
		Wish(t, fluent.WrapNode(n).TraverseIndex(0).ReprKind(), ShouldEqual, ipld.ReprKind_String)
	...
	t.Run("nested list node", func(t *testing.T) {
		...
		Require(t, fluent.WrapNode(n).TraverseIndex(0).ReprKind(), ShouldEqual, ipld.ReprKind_List)
		Wish(t, fluent.WrapNode(n).TraverseIndex(0).Length(), ShouldEqual, 1)
		Wish(t, fluent.WrapNode(n).TraverseIndex(0).TraverseIndex(0).ReprKind(), ShouldEqual, ipld.ReprKind_String)
